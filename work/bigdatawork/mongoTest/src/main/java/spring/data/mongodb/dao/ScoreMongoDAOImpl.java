...
//spring-data에서 지원되는 Repository와 MongoTemplate클래스를 이용해서 mongodb에 액세스하는 기능을 구현
...
public class ScoreMongoDAOImpl implements ScoreMongoDAO {
	...
	//mongodb를 연동하기 위한 기능을 제공(spring-data-mongodb 라이브러리)
	@Autowired
	...
	public List<ScoreDTO> findCriteria(String key, String value) {
		...
		/*
		 * //query클래스의 addCriteria 메소드와 criteria클래스의 where메소드의 활용
		...
		*/
		...
		List<ScoreDTO> doclist = mongoTemplate.find(query, ScoreDTO.class, "score");
		return doclist;
	}
	/*@Override
	public List<ScoreDTO> findCriteria(String key, String value) {
	...
	public List<ScoreDTO> findAll(int pageNo) {
		 ...
		 * [리턴]
		 * findAll이 처리되면 페이징된 객체를 Page타임으로 리턴한다.
		...
		*/
		...
		//getContent()는 Page객체가 갖고 있는 페이징된 리스트객체를 반환
