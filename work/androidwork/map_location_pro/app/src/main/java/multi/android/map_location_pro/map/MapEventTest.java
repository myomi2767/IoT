...
/*
    지도화면이 출력된 후에 이벤트를연결할 수 있어야 한다. 지도를 클릭하거나 지도를 드래그,
    지도를 줌레벨버튼을 이용해서 줌레벨을 변경

    OnMapClickListener => 지도를 클릭할 때 발생하는 이벤트에 대한 처리
    OnMapLongClickListener => 맵을 길게 눌렀을 때 발생하는 이벤트에 대한 처리
    OnCameraMoveListener => 지도의 위치가 바뀌거나 줌레벨이 변경되어 발생하는 이벤트에 대한 처리
    OnCameraMoveStartedListener => 지도의 위치가 바뀌거나 줌레벨이 변경되어 카메라가 이동되기 시작할 때
                                   이벤트에 대한 처리
    [순서]
...
public class MapEventTest extends AppCompatActivity implements OnMapReadyCallback,
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        if(map!=null){
            ...
            //현재 나의 위치를 포인트로 표시 - 위치 기반서비스에 대한 퍼미션체크가 완료되어야 표시
    ...
    public void onMapClick(LatLng latLng) {
        map.clear();
        Toast.makeText(this,"지도를 클릭했습니다 => 위도:"
                +latLng.latitude+",경도:"+latLng.longitude, Toast.LENGTH_SHORT).show();
    ...
    public void onMapLongClick(LatLng latLng) {
        Toast.makeText(this,"지도를 길게 클릭했습니다 => 위도:"
                +latLng.latitude+",경도:"+latLng.longitude, Toast.LENGTH_SHORT).show();
    }

    @Override
    public void onCameraMove() {
        Toast.makeText(this,"카메라가 이동합니다", Toast.LENGTH_SHORT).show();
    }

    @Override
    public void onCameraMoveStarted(int i) {
        Toast.makeText(this,"카메라가 이동이 시작합니다", Toast.LENGTH_SHORT).show();
    }
}
